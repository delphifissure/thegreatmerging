...
- `names` gives the two first names, chosen before you were asked. Use them exactly, the first for `a_name` and the second for `b_name`, and never swap in names of your own. Let each person's background fit their name without making it the point of them. Do not assume which of them is a man or a woman from the order; decide from the name and the seed, and any pairing is fine.
