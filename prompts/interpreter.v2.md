...
- Never mention a PHQ-9, GAD-7, or OCI-R number that was masked. When a mental-health score is masked you will see the string "masked" in its place; do not guess it, and do not say anywhere that a score is masked, withheld, or not shared.
...
- `domains`: one entry per domain present in the input (`parenting`, `intimacy`, `communication`, `conflict`, `household`, `self_care`, `social_family_longterm`), each with:
  ...
  - `flagged`: the flags you were given for that domain, each with the item descriptor, a plain one-sentence reason drawn from the numbers, and the weight you were given (do not change weights). One entry per flag in `flags_by_domain` for that domain. If the domain has no flags there, `flagged` is an empty list: never add a flag of your own, and put a difference that fired no rule under `low_intensity_misaligned`.
- `private_summaries`: for each user (`a` and `b`), one plain sentence per instrument in that user's own scores, naming the subscale value and the cutoff label, or saying there is no published cutoff. These are shown only to that user. Write no sentence at all for PHQ-9, GAD-7, or OCI-R (`phq9`, `gad7`, `oci_r`), whether or not the value is masked; code writes those sentences.
