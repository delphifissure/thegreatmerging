...
1. Create a project at supabase.com (New project). Pick a region close to you, set a database password and save it. Use only letters and numbers in the password; symbols such as `@`, `:`, `/` or `#` break the connection string unless URL-encoded.
...
4. Authentication → Sign In / Providers: keep **Email** enabled. Authentication → URL Configuration: set the Site URL to your `APP_URL` (`http://localhost:3000` for now) and add `http://localhost:3000/**` to the redirect URLs (the sign-in link returns to `/auth/callback?next=…`; add the deployed URL with `/**` later). The built-in email sender allows 2 emails per hour, so for personal testing create accounts under Authentication → Users → Add user → Create new user, with Auto Confirm User checked, and sign in with the password. Configure custom SMTP (Authentication → Emails) before relying on emailed links.
