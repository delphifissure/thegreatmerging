...
Built on 18 September 2026 at `/sandbox`: two made-up people, written by hand or generated from a few words, each with a life history of the kind a therapist would hold in their notes, plus the history they share. The notes are written about the two of them; each avatar is given a brief written to it in the second person, made from its own notes, the shared history and the situation and never from the other's notes, which whoever set it up can read and edit before anyone speaks. A scenario also has a private side for each person, what only they know or believe right now, which reaches their own brief and nowhere else; a brief never contains what its person could not know, because an avatar that is told the plot knows the plot. Before each turn an avatar writes what it felt and what it wants, which whoever runs the sandbox can read. Nothing the two say is filtered: they are invented, so the guardrail that protects real people from being characterized does not apply, and the only stop is violence or self-harm. Whoever set it up reads both sides, sees what each avatar was told, and sees each turn coded as a move with how it was meant and how it landed. When the two people are generated, the bones of each life are drawn in code from long lists (the household they grew up in, how feeling went in that house, a turning point, how they argue and come back, how they talk, what they are afraid of, what the untold thing is about, where they live) and the model writes on top of them, because a model left to choose writes the same childhood and the same way of arguing every time; names are drawn the same way, and whoever set it up can read what was drawn. It exists to look at the conversational machinery without anyone's real material in it: whether avatars hold their positions, whether a secret stays a secret, whether two runs of the same scene resemble each other. It says nothing about fidelity to a real person, which is what the replay is for.
