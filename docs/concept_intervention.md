...
Built on 18 September 2026 behind `BIOGRAPHER_ENABLED=1`: the biographer, the drafter, ratification of history and constitution lines with settled and open marks, and the one-notch-ahead self with a "sounds like me" rating on every reply and a hand-off of what it could not answer back to the biographer.
...
How an avatar should sound, including registers, imported writing samples and the gap between what is meant and how it lands, is in `concept_voice.md`. Its first three items are built: avatars are shown samples of how their person writes, chosen by register; a person can paste their own side of things they wrote at `/documents/voice`; and every one-notch-ahead reply takes two verdicts, how it sounds and what it says, plus the person's own wording.

The first test of the rehearsal idea is built at `/replay`: the two of you separately describe a past argument, your avatars replay it from how it started, and each of you says whether your avatar moved the way you did. Each person reads only their own avatar's words and the coded moves of the other's, unless both choose to watch it together; then both read everything, coach their own avatar turn by turn, and run it again. See "How to find out cheaply whether it works" in `concept_simulation.md`.
