...
import { ScenarioSchema, type SandboxTurn, type Scenario, type Side } from "@/lib/sandbox/scenario";

export type Sandbox = { threadId: string; scenario: Scenario; turns: SandboxTurn[]; maxTurns: number; created_at: Date };
...
export async function readSandbox(threadId: string, userId: string): Promise<Sandbox | null> {
  ...
  return { threadId: thread.id, scenario, turns, maxTurns: scenario.maxTurns + extra, created_at: thread.created_at };
}
