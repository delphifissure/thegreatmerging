...
export async function deleteFixture(sql: Sql, userIds: string[], coupleIds: string[]): Promise<void> {
  ...
  await sql`delete from replay_turn_words where user_id in ${sql(u)}`;
  await sql`delete from replay_turns where speaker_id in ${sql(u)}`;
  await sql`delete from replay_accounts where user_id in ${sql(u)}`;
  await sql`delete from replays where proposer_id in ${sql(u)} or partner_id in ${sql(u)}`;
