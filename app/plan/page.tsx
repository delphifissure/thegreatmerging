...
export default async function PlanPage() {
  ...
  const header = <PageHeader title="The plan" lede="Together, one topic per sitting. For each: what you agreed, what each of you does, and when you look at it again." />;
  ...
  return (
    <div className="space-y-6">
      {header}
      <p className="reading max-w-prose text-[16px] text-muted">
        {latest ? `Version ${latest.version}. Every save creates a new version.` : "No plan yet. The topics below come from the brief and from what came up; edit freely, then save version 1."} Anything you both called a requirement is settled and sits at the top. Where one of you said requirement and the other preference, pick a date to look at it again. Parked topics stay parked.
      </p>
      ...
      {latest && previous ? (
        <Card>
          <h2 className="text-[22px]">
            What changed since version {previous.version}
          </h2>
          {diff.length === 0 ? (
            <p className="mt-2 text-sm text-muted">Nothing changed.</p>
          ) : (
      ...
      <Card>
        <h2 className="text-[22px]">Export</h2>
        <p className="mt-1 text-sm text-muted">The plan or the brief, as Markdown or PDF. Scores carry the questionnaire they came from, and anything the app wrote is marked as such.</p>
